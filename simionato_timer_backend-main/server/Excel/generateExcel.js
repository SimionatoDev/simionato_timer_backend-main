...
async function generateExcel(lsApontamentos) {
  ...
  try {
    ...
    sheet.mergeCells("D5");
    // Adiciona o título na célula D3
    ...
    for await (const registro of lsApontamentos) {
      ...
      const horaInicial = registro.inicial
        ? new Date(registro.inicial).getHours()
        : 0;
      const minutosInicial = registro.inicial
        ? new Date(registro.inicial).getMinutes()
        : 0;

      // Extrair apenas a hora e os minutos da data final
      const horaFinal = registro.final
        ? new Date(registro.final).getHours()
        : 0;
      const minutosFinal = registro.final
        ? new Date(registro.final).getMinutes()
        : 0;
      ...
      console.log(
        "Valor de horasapon após conversão para número:",
        horasApontadasNumero
      );

      const horasApontadasDecimal =
        converterParaHoraMinuto(horasApontadasNumero);
      console.log(
        "Valor de horas e minutos após a conversão para decimal:",
        horasApontadasDecimal
      );
    ...
    const auditores = Array.from(
      new Set(lsApontamentos.map((registro) => registro.exec_razao))
    );
    ...
    sheet.getCell(`B${rowIndex}`).value = "Auditor";
    sheet.getCell(`C${rowIndex}`).value = "Total de Horas";
    sheet.getCell(`B${rowIndex}`).style = titleStyle;
    sheet.getCell(`C${rowIndex}`).style = titleStyle;
    rowIndex++;
    ...
    auditores.forEach((auditor, index) => {
      ...
      sheet.getCell(`C${rowIndex}`).value =
        converterParaHoraMinuto(totalHorasPorAuditor);
      sheet.getCell(`B${rowIndex}`).style = {
        border: { bottom: { style: "thin" } },
      };
      sheet.getCell(`C${rowIndex}`).style = {
        border: { bottom: { style: "thin" } },
      };
    ...
    const totalGeral = Object.values(totalPorAuditor).reduce(
      (acc, curr) => acc + curr,
      0
    );
    sheet.getCell(`B${rowIndex}`).value = "Total Geral";
    sheet.getCell(`C${rowIndex}`).value = converterParaHoraMinuto(totalGeral);
    sheet.getCell(`B${rowIndex}`).style = totalStyle;
    sheet.getCell(`C${rowIndex}`).style = totalStyle;
    ...
    sheet.columns.forEach((column, columnIndex) => {
      ...
      sheet.getColumn(columnIndex + 1).width =
        maxLength < 10 ? 10 : maxLength + 2; // Adiciona uma folga de 2 caracteres
    });
